...
class SMTP{
     ...
     * Alternatively, you can provide a callable expecting two params: a message string and the
     * debug level:
     *
     * ```php
     ...
     * This needs to be quite high to function correctly with hosts using greetdelay as an
     * anti-spam measure.
     *
    ...
    protected function getSMTPConnection($host, $port = NULL, $timeout = 30, $options = []){
        ...
        if (strpos(PHP_OS, 'WIN') !== 0){
            ...
            if (0 !== $max && $timeout > $max && strpos(ini_get('disable_functions'),
     ...
     * @param OAuthTokenProvider $OAuth    An optional OAuthTokenProvider instance for XOAUTH2
     *                                     authentication
     *
    ...
    public function authenticate(
        ...
        if (array_key_exists('EHLO', $this->server_caps)){
            ...
            if (NULL !== $authtype && !in_array($authtype, $this->server_caps['AUTH'], TRUE)){
                $this->edebug('Requested auth method not available: ' . $authtype,
                    self::DEBUG_LOWLEVEL);
                $authtype = NULL;
            }
    ...
    protected function sendCommand($command, $commandstring, $expect){
        ...
        $matches = [];
        if (preg_match('/^([\d]{3})[ -](?:([\d]\\.[\d]\\.[\d]{1,2}) )?/', $this->last_reply,
            $matches)){
            $code    = (int) $matches[1];
     ...
     * @param string $command Optionally, the command this is part of, used only for controlling
     *                        debug output
     *
     ...
     *
     * @param string|callable $method The name of the mechanism to use for debugging output, or a
