 ...
 * @license   https://www.gnu.org/licenses/old-licenses/lgpl-2.1.html GNU Lesser General Public
 *            License
...
class PHPMailer{
     ...
     *
     * @see https://www.rfc-editor.org/rfc/rfc3461.html#section-4.1 for more information about
     *      NOTIFY
     */
    public $dsn = '';
     ...
     * By default PHPMailer will use `echo` if run from a `cli` or `cli-server` SAPI, `html`
     * otherwise. Alternatively, you can provide a callable expecting two params: a message string
     * and the debug level:
     *
     * ```php
    ...
    protected function edebug($str){
        ...
        if (is_callable($this->Debugoutput) && !in_array($this->Debugoutput,
                ['error_log', 'html', 'echo'])){
    ...
    /**
     * Parse and validate a string containing one or more RFC822-style comma-separated email
     * addresses of the form "display name <address>" into an array of name/address pairs. Uses the
     * imap_rfc822_parse_adrlist function if the IMAP extension is available. Note that quotes in
     * the name part are removed.
     *
     * @see https://www.andrew.cmu.edu/user/agreen1/testing/mrbs/web/Mail/RFC822.php A more careful
     *      implementation
     *
     ...
     * * `eai` Use a pattern similar to the HTML5 spec for 'email' and to firefox, extended to
     * support EAI (RFC6530).
     ...
     * ```
     *
     * You can also set the PHPMailer::$validator static to a callable, allowing built-in methods
     * to use your validator.
     *
    ...
    public function preSend(){
        if (
            'smtp' === $this->Mailer
            || ('mail' === $this->Mailer && (\PHP_VERSION_ID >= 80000 || stripos(PHP_OS,
                        'WIN') === 0))
        ){
     ...
     * Note that escapeshellarg and escapeshellcmd are inadequate for our purposes, especially on
     * Windows.
     *
    ...
    protected function mailSend($header, $body){
        ...
        if ($this->SingleTo && count($toArr) > 1){
            ...
            $this->doCallback($result, $this->to, $this->cc, $this->bcc, $this->Subject, $body,
                $this->From, []);
        }
        if (isset($old_from)){
    ...
    public function setLanguage($langcode = 'en', $lang_path = ''){
        ...
        if (
            !preg_match('/^(?P<lang>[a-z]{2})(?P<script>_[a-z]{4})?(?P<country>_[a-z]{2})?$/',
                $langcode, $matches)
            && $langcode !== 'en'
        ){
            $foundlang = FALSE;
     ...
     *                     containing an address and element 1 containing a name, like:
     ...
     * If you need to do that, fetch the resource yourself and pass it in via a local file or
     * string.
     *
     * @param string $path        Path to the attachment
     * @param string $name        Overrides the attachment name
     * @param string $encoding    File encoding (see $Encoding)
     * @param string $type        MIME type, e.g. `image/jpeg`; determined automatically from $path
     *                            if not specified
    ...
    public function encodeHeader($str, $position = 'text'){
        ...
        switch ($encoding){
            case 'B':
                ...
                $encoded = preg_replace('/^(.*)$/m', ' =?' . $charset . "?$encoding?\\1?=",
                    $encoded);
                break;
            case 'Q':
     ...
     * the HTML refers to using the `$cid` value in `img` tags, for example `<img
     * src="cid:mylogo">`. Never use a user-supplied path to a file!
     *
     * @param string $path        Path to the attachment
     ...
     * @param string $type        File MIME type (by default mapped from the `$path` filename's
     *                            extension)
     ...
     * This can include images, sounds, and just about any other document type.
     * If your filename doesn't contain an extension, be sure to set the $type to an appropriate
     * MIME type.
     *
     ...
     * @param string $encoding    File encoding (see $Encoding), defaults to 'base64'
     * @param string $type        MIME type - will be used in preference to any automatically
     *                            derived type
     * @param string $disposition Disposition to use
     *
     * @return bool True on successfully adding an attachment
    ...
    public static function isValidHost($host){
        ...
        if (strlen($host) > 2 && substr($host, 0, 1) === '[' && substr($host, - 1, 1) === ']'){
            return filter_var(substr($host, 1, - 1), FILTER_VALIDATE_IP,
     ...
     * $basedir is prepended when handling relative URLs, e.g. <img src="/images/a.png"> and must
     * not be empty will look for an image file in $basedir/images/a.png and convert it to inline.
     * If you don't provide a $basedir, relative paths will be left untouched (and thus probably
     * break in email) Converts data-uri images into embedded attachments. If you don't want to
     * apply these transformations to your HTML, just set Body and AltBody directly.
     *
     * @param string        $message  HTML message string
     * @param string        $basedir  Absolute path to a base directory to prepend to relative
     *                                paths to images
    ...
    public function DKIM_Add($headers_line, $subject, $body){
            ...
            foreach ($copiedHeaders as $copiedHeader){
                ...
                if (strlen($copiedHeader) > self::STD_LINE_LENGTH - 3){
                    $copiedHeaderFields .= substr(
                        chunk_split($copiedHeader, self::STD_LINE_LENGTH - 3,
                            static::$LE . self::FWS),
                        0,
        ...
        $signature            = trim(chunk_split($signature, self::STD_LINE_LENGTH - 3,
            static::$LE . self::FWS));
    ...
    public static function hasLineLongerThanMax($str){
        return (bool) preg_match('/^(.{' . (self::MAX_LINE_LENGTH + strlen(static::$LE)) . ',})/m',
            $str);
    }
    ...
    protected function doCallback($isSent, $to, $cc, $bcc, $subject, $body, $from, $extra){
        if (!empty($this->action_function) && is_callable($this->action_function)){
            call_user_func($this->action_function, $isSent, $to, $cc, $bcc, $subject, $body, $from,
                $extra);
        }
    }
