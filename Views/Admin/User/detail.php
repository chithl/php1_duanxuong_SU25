						...
						<table class="table table-bordered">
							...
							<tr>
								...
								<td>
                                   ...
                                   <?= $result['status'] === 'active' ? 'Đang hoạt động' : 'Đã khóa' ?>
                                   </span>
								</td>
							</tr>
							<tr>
                                ...
                                <span class="badge <?= $result['role'] === 'admin' ? 'bg-primary' : 'bg-secondary' ?>">
                                    <?= $result['role'] === 'admin' ? 'Quản trị viên' : 'Người tiêu dùng' ?>
                                </span>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<th>Ngày tạo</th>
								<td>
                                    <?= !empty($result['created_at']) ? date('d/m/Y H:i:s',
                                        strtotime($result['created_at'])) : '<i>Không có</i>' ?>
								</td>
							</tr>
							<tr>
								<th>Ngày cập nhật</th>
								<td>
                                    <?= !empty($result['updated_at']) ? date('d/m/Y H:i:s',
                                        strtotime($result['updated_at'])) : '<i>Không có</i>' ?>
								</td>
							</tr>
						</table>
						<a href="?page=user" class="btn btn-secondary">Quay lại danh sách</a>
						<a href="?page=user&action=edit&id=<?= $result['id'] ?>" class="btn btn-outline-warning">Sửa</a>
					</div>
				</div>
			</div>
		</div>
